...
class ProteinGraphDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        protein_1_graph = torch.load(row["protein_1_graph"][self.graph_type])
        protein_2_graph = torch.load(row["protein_2_graph"][self.graph_type])
...
def train(data_dir, model_name, graph_type, learning_rate, weight_decay, step_size, gamma, num_epochs, batch_size, dropout, device, output_dir):
    ...
    num_features = 20 if graph_type == "onehot" else (7 if graph_type == "physchem" else 1024)
    ...
    # Test evaluation
    test_loss, test_accuracy, test_precision, test_recall, test_f1 = evaluate_one_epoch(model, criterion, dataloaders["test"], device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--graph_type", type=str, required=True, choices=["onehot", "physchem", "protbert", "prostt5"], help="Graph type to use.")
